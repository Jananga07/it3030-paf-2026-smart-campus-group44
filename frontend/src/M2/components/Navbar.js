        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-6 pb-4 flex flex-col gap-4">
          ...
          {user && user.role === 'ADMIN' && (
            <Link to="/admin-dashboard" onClick={() => setOpen(false)}
              className={`text-sm font-medium ${pathname === '/admin-dashboard' ? 'text-indigo-600' : 'text-gray-500'}`}>
              🛠 Admin
            </Link>
          )}
          {user ? (
            <>
              ...
              <button onClick={() => { logout(); setOpen(false); }}
                className="text-sm font-medium text-red-500 text-left">Logout</button>
            </>
          ) : (
            <>
              <button onClick={() => { navigate('/login'); setOpen(false); }}
                className="text-sm font-medium text-indigo-600 text-left">User Login</button>
              <button onClick={() => { navigate('/admin-login'); setOpen(false); }}
                className="text-sm font-medium text-gray-600 text-left">Admin Login</button>
            </>
          )}
        </div>
      )}
